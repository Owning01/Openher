...
// Per-directory scan cache. `listSkills` previously re-read every SKILL.md
// (readFile + frontmatter parse + examples scan) on each /api/skills request,
// which is the daemon's biggest steady-state disk/CPU+RAM churn given ~160
// skills. We cache the parsed entry keyed by directory and invalidated by the
// file's mtime, so unchanged skills are served from memory. The workspace
// scoping pass still runs over the cached objects, so visibility semantics are
// unchanged. Import/update/delete blow the cache explicitly.
interface CachedSkillEntry {
  parent: SkillInfo;
  derivedExamples: DerivedExample[];
}

const skillEntryCache = new Map<string, { mtimeMs: number; entry: CachedSkillEntry }>();

export function invalidateSkillsCache(): void {
  skillEntryCache.clear();
}

async function readSkillEntry(dir: string, source: SkillSource): Promise<CachedSkillEntry | null> {
  ...
  const cached = skillEntryCache.get(dir);
  if (cached && cached.mtimeMs === stats.mtimeMs) return cached.entry;
  ...
  const parentId =
    typeof data.name === "string" && data.name ? data.name : path.basename(dir);
  ...
  const parent: SkillInfo = {
    ...
    aggregatesExamples: derivedExamples.length > 0,
    critiquePolicy: normalizeCritiquePolicy(data.od?.critique?.policy),
    body: parentBody,
    dir,
  };
  const entry: CachedSkillEntry = { parent, derivedExamples };
  skillEntryCache.set(dir, { mtimeMs: stats.mtimeMs, entry });
  return entry;
}
...
export async function listSkills(
  ...
  for (let rootIdx = 0; rootIdx < roots.length; rootIdx += 1) {
    ...
    for (const entry of entries) {
      ...
      const skillPath = path.join(dir, "SKILL.md");
      let built: CachedSkillEntry | null = null;
      try {
        built = await readSkillEntry(dir, source);
      } catch {
        continue;
      }
      if (!built) continue;
      const { parent, derivedExamples } = built;
      ...
      // root wins so user shadows built-in.
      ...
      for (const example of derivedExamples) {
        const derivedId = `${parent.id}:${example.key}`;
        ...
        out.push({
          ...
          ...(parent.descriptionI18n ? { descriptionI18n: parent.descriptionI18n } : {}),
          ...
          previewType: parent.previewType,
          designSystemRequired: parent.designSystemRequired,
          defaultFor: [],
          upstream: parent.upstream,
          featured: null,
          fidelity: parent.fidelity,
          speakerNotes: parent.speakerNotes,
          animations: parent.animations,
          examplePrompt: parent.examplePrompt,
          ...(parent.examplePromptI18n ? { examplePromptI18n: parent.examplePromptI18n } : {}),
          ...
          critiquePolicy: parent.critiquePolicy,
