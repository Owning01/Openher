...
- **`dropEffect` incompatible mata el `drop` en silencio:** los dragstarts internos ponen `effectAllowed = "move"` (`FileRow`, filas del explorer, cards) pero el Composer forzaba `dropEffect = "copy"` en `dragover` → el navegador NO dispara `drop` (el overlay "Drop to add to Agent" igual se muestra y engaña). Regla: en `dragover`, `dropEffect = effectAllowed === "move" ? "move" : "copy"` (patrón ya usado en `shellPanels:1278`, `TabBar:273`); test en `ComposerDrop.test.tsx`. No reproducible en jsdom (no hace cumplir la regla) — se depuró en vivo vía CDP `:9333`.
