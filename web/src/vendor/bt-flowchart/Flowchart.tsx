// BeautifulUI (MIT) — https://beautifului.dev/ — fork para el chat de OpenHer.
// Id del catálogo anim-lab: bt-flowchart. Se renderiza desde un bloque
// ```flowchart (JSON con steps/edges) en components/Markdown.tsx.
...
const EDGES: Edge[] = [{ from: "trigger", to: "cond" }];
...
function Menu({
  ...
  return (
    <div
      ...
      style={{
        animation: "bt-fc-pop-in 180ms cubic-bezier(0.23,1,0.32,1) both",
...
/* ── the canvas ── */
export function Flowchart({ steps = NODES, edges = EDGES }: { steps?: StepNode[]; edges?: Edge[]; variant?: string }) {
  ...
  return (
    <div
      ref={canvasRef}
      className="bt-fc relative w-full select-none overflow-hidden rounded-card bg-page shadow-hairline"
      style={{
        height: canvasH,
