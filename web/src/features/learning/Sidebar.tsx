      ...
      <div className="learning-cat-header-row" style={{ display: "flex", alignItems: "center", gap: 4 }}>
       <button type="button" onClick={() => toggleCat(cat.id)} className="learning-cat-toggle" aria-expanded={openCats.has(cat.id)} style={{ flex: 1 }}>
        ...
        <span style={{ flex: 1, display: "inline-flex", alignItems: "center", gap: 6 }}>
         <span aria-hidden="true" style={{ fontSize: 12 }}>F</span>
         <span>{cat.title}</span>
       ...
       <button type="button" className="btn-icon compact" title="Crear documento vacío" aria-label="Crear documento vacío" style={{ flexShrink: 0, width: 26, height: 26, fontSize: 12 }} onClick={() => onCreateEmptyDoc?.(cat.id)}>Doc</button>
      </div>
      {openCats.has(cat.id) && (
         ...
         return (
          <li key={item.id} style={{ position: "relative" }}>
           ...
           >
            ...
            <span className="learning-check" aria-hidden="true">{done ? "OK" : ""}</span>
            <span aria-hidden="true" style={{ fontSize: 12, flexShrink: 0 }}>{item.isCustom ? "Doc" : "Doc"}</span>
